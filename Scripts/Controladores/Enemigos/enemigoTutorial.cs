...
public class enemigoTutorial : MonoBehaviour {
	...
	// Este script controla el comportamiento del primer enemigo del tutorial.
	...
	public int vidaEnemigo = 6;					// la vida del enemigo
	...
	void Start () 
	{
		GetComponent<Rigidbody>().AddForce(transform.forward * 280);
	...
	void FixedUpdate ()
	{
			...
			{
				GetComponent<Rigidbody>().AddForce(transform.forward * -160 * Time.fixedDeltaTime);
				rebote = true;
